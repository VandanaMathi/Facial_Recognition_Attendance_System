...
class SQL:
    ...
    def update(self, sid, status):
        self.mydb = self.DB.cursor()
        self.sql_q = "INSERT INTO Attendance_"+status+" VALUES (\""+sid+"\",NOW())"
        try:
            self.mydb.execute(self.sql_q)
            self.DB.commit()
        except mysql.connector.Error as err:
            print("Something went wrong: {}".format(err))

    def new_id(self, sid, ename, edesg):
        self.mydb = self.DB.cursor()
        self.sql_q = "INSERT INTO Students VALUES (\"" + sid + \
            "\",\"" + ename + "\"," + "\"" + edesg + "\",NOW())"
        try:
            self.mydb.execute(self.sql_q)
            self.DB.commit()
...
class Train(SQL):
    ...
    def train(self):
        ...
        for dir in os.listdir(self.image_source):
            for i in range(10):
                face = np.array(Image.open(self.image_source +
                                           dir+"/"+str(i+1)+".jpg").convert('L'),  'uint8')
        ...
        recognizer.train(sampling, np.array(s_id))
        recognizer.save(self.reference)
        print("\nTraining Complete.")
        print("Total Person Trained is {}".format(
            len(os.listdir(self.image_source))))

    def enroll(self):
        self.name = input("Enter the Name : ")
        self.desg = input("Enter Branch Name : ")
        ...
        camera = cv2.VideoCapture(self.cam_index, cv2.CAP_DSHOW)
        ...
        while 1:
            ...
            for x, y, w, h in output:
                cv2.imwrite("Images/" + self.emp_id + "/" +
                            str(no) + ".jpg", gray[y:y+h, x:x+w])
                no += 1
...
class Recognise(Train):
    def run(self):
        ...
        while (1):
            ...
            for x, y, w, h in output:
                s_id, confidence = recognizer.predict(gray[y:y + h, x:x + w])
                cv2.rectangle(frame, (x, y), (x + w, y + h), (255, 0, 0), 1)
                if (confidence > 100):
                    s_id = -1
                ...
                if (s_id != -1) and (prev != s_id):
                    SQL.update(self, self.user_list[s_id], self.status)
                    prev = s_id

                cv2.putText(frame, str(
                    self.user_list[s_id]), (x + 5, y - 5), cv2.FONT_HERSHEY_PLAIN, 1, (255, 255, 255), 2)
